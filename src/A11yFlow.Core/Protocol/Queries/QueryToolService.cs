...
public sealed class QueryToolService
{
    ...
    private A11yFlow.Core.Snapshots.SnapshotResult? _activeWindowSnapshot;
    private A11yFlow.Core.Snapshots.SnapshotResult? _focusContextSnapshot;
    private readonly LinkedList<A11yFlow.Core.Snapshots.SnapshotResult> _recentWindowSnapshots = [];
    ...
    public ToolResult<A11yFlow.Core.Snapshots.SnapshotResult> WindowsSnapshot(WindowsSnapshotRequest request)
    {
        try
        {
            ...
            var normalizedSnapshot = new A11yFlow.Core.Snapshots.SnapshotResult(
                snapshot.WindowRef,
            ...
            return new ToolResult<A11yFlow.Core.Snapshots.SnapshotResult>(normalizedSnapshot, null);
        }
        catch (InvalidOperationException ex)
        {
            return new ToolResult<A11yFlow.Core.Snapshots.SnapshotResult>(null, new ToolError(
    ...
    private static A11yFlow.Core.Refs.ElementRef? FindFocusedRef(ElementNode root)
    {
    ...
    private ToolResult<A11yFlow.Core.Snapshots.SnapshotResult> ResolveScopedSnapshot(SelectorScope scope)
    {
    ...
    private static A11yFlow.Core.Refs.WindowRef? TryExtractWindowRef(string reference)
    {
        ...
        return new A11yFlow.Core.Refs.WindowRef(windowPart);
    }

    private A11yFlow.Core.Snapshots.SnapshotResult GetActiveSnapshot()
    {
    ...
    private A11yFlow.Core.Snapshots.SnapshotResult GetSnapshot(A11yFlow.Core.Refs.WindowRef windowRef, bool preferActiveCache)
    {
    ...
    private A11yFlow.Core.Snapshots.SnapshotResult RefreshFocusSnapshot(A11yFlow.Core.Refs.WindowRef windowRef)
    {
        var snapshot = _snapshotBuilder.BuildWindowSnapshot(windowRef);
    ...
    private void RememberSnapshot(A11yFlow.Core.Snapshots.SnapshotResult snapshot)
    {
    ...
    private A11yFlow.Core.Snapshots.SnapshotResult? ResolveDescribeSnapshot(A11yFlow.Core.Refs.WindowRef windowRef, string reference)
    {
    ...
    private static bool SnapshotContainsRef(
        A11yFlow.Core.Snapshots.SnapshotResult? snapshot,
        A11yFlow.Core.Refs.WindowRef windowRef,
        string reference)
    {
    ...
    private string GetSnapshotCacheName(A11yFlow.Core.Snapshots.SnapshotResult snapshot)
    {
    ...
    private static bool IsCachedActiveSnapshot(A11yFlow.Core.Refs.WindowRef windowRef, A11yFlow.Core.Snapshots.SnapshotResult? snapshot)
    {
    ...
    private static string BuildFocusSummary(
        ...
        IReadOnlyList<A11yFlow.Core.Refs.ElementRef> parentChain,
        IReadOnlyList<A11yFlow.Core.Refs.ElementRef> siblingRefs,
        IReadOnlyList<A11yFlow.Core.Refs.ElementRef> childRefs)
    {
