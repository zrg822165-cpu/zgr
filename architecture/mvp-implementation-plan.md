# OpenClaw 一期 MVP 实施方案

## 1. 目标

在现有架构草案基础上，定义 `openclaw` 一期 `MVP` 的可实施方案，目标不是覆盖所有桌面软件，而是尽快打通一条稳定闭环：
...
- 一个服务于 OpenClaw 上层的桌面操作 substrate
...
以下能力默认属于 OpenClaw 上层，不应继续下沉到底层：
...
- 已输出候选、最佳命中、歧义状态和诊断信息，面向 `openclaw` 的文本视觉消费
...
- 为满足 `openclaw` 的文本操作闭环，最小 `dry_run` 与有限 `input_simulation` fallback 已提前接入
...
- 使用 OpenClaw 主路径对若干真实桌面软件执行任务时，能够明确区分“产品可用”、“产品待打磨”和“需要未来视觉产品线”的边界
...
`openclaw` 一期最重要的不是做成“大而全的桌面自动化框架”，而是先做成一个对 agent 真正可用、能操作真实软件的最小执行内核：
