...
src/
  Allyflow.Core/
  Allyflow.Infrastructure.Windows/
tests/
  Allyflow.Tests.Unit/
  Allyflow.Tests.Integration/
```

建议职责分配：

- `Allyflow.Core`: 核心模型、接口、错误码、执行策略，以及工具请求/结果与协议适配逻辑
- `Allyflow.Infrastructure.Windows`: `FlaUI/UIA3` 适配、窗口发现、快照、动作执行
- `Allyflow.Tests.*`: 单元、集成，以及集成项目内聚合的测试夹具
...
- `Allyflow` 当前主产品明确为 `accessibility_tree` 优先的桌面自动化能力
...
- 安装 `.NET SDK 8.0.420`
- 创建 `Allyflow.sln`
- 创建 `src/Allyflow.Core`
- 创建 `src/Allyflow.Infrastructure.Windows`
- 创建 `tests/Allyflow.Tests.Unit`
- 创建 `tests/Allyflow.Tests.Integration`
- 补充项目引用与基础依赖
...
- 通过 `dotnet build Allyflow.sln --configuration Release` 与 `dotnet test Allyflow.sln --configuration Release`
...
- 通过 `dotnet build Allyflow.sln --configuration Release`
- 通过 `dotnet test tests/Allyflow.Tests.Unit/Allyflow.Tests.Unit.csproj --configuration Release`
- 通过 `dotnet test Allyflow.sln --configuration Release`
...
- 直接把 Allyflow 当产品使用，用它去操作真实电脑软件
...
- Allyflow 是否已经真实赋能了典型 Windows 软件操作
...
Allyflow 现在已经不再处于“先把能力拼出来”的阶段，而是进入“用产品验证产品”的阶段。
