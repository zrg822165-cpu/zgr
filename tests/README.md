# OpenClaw Test Directory

This directory contains the automated tests for the current OpenClaw implementation.

## Current contents

- `OpenClaw.Tests.Unit/`: unit coverage for selector parsing, locating, action orchestration, diagnostics, and recovery behavior
- `OpenClaw.Tests.Integration/`: real WinForms/UIA-backed integration coverage for query, action, scenario, cache/refresh, and tool-contract failure behavior
...
- The current fixture-backed tests validate the text-structured product path: snapshots, `ref`, selectors, structured actions, explainable diagnostics, and recoverable failure surfaces. The shared WinForms fixture host now lives directly inside `OpenClaw.Tests.Integration/`.
