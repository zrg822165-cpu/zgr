...
- `ActionResult` 字段稳定
- 局部事件刷新逻辑成立
- 在若干真实桌面软件上，Allyflow 能沿 `accessibility_tree` 主路径完成基本任务并给出可解释结果

当前阶段说明：

- 本矩阵仍然服务于 Allyflow 一期主产品
...
一期测试的重点不是覆盖所有桌面软件，也不是提前证明视觉 fallback，而是验证 `allyflow` 这条核心路径是否已经具备真实产品价值：
